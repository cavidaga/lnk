...
if (IS_PROD && !RAW_JWT_SECRET) {
  throw new Error('[auth] AUTH_JWT_SECRET is required in production');
}

// In development, fall back to an insecure default to avoid breaking local login
const JWT_SECRET = RAW_JWT_SECRET || 'dev-insecure-secret';
...
export async function requireAuth(req, res) {
  ...
  try {
    if (user.passwordChangedAt && session.iat) {
      const changedAt = Math.floor(new Date(user.passwordChangedAt).getTime() / 1000) || 0;
      if (changedAt && session.iat < changedAt) {
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
