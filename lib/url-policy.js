...
export const DEFAULT_BLOCKED_HOSTS = [
  ...
  // Large PDFs direct
  "arxiv.org",
  // Google services (not suitable for news analysis)
  "*.google.com","google.com","www.google.com",
  // Sites that consistently block LNK access
  "oxu.az","www.oxu.az",
];
...
export function isBlockedPath(urlLike) {
  try {
    ...
    // Block live news content (actively edited, not suitable for bias analysis)
    if (/live-news|live-updates|breaking-news|live-blog|live-coverage/i.test(u.pathname)) return true;
