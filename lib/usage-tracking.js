...
export async function getUserUsage(userId, period = 'month') {
  ...
  try {
    if (period === 'month') {
      ...
      const data = monthlyData || {}; // Handle null case
      return {
        period,
        startDate,
        endDate,
        requests: parseInt(data.requests || 0),
        successfulRequests: parseInt(data.successful_requests || 0),
        failedRequests: parseInt(data.failed_requests || 0),
      ...
      for (const key of keys) {
        ...
        const keyData = data || {}; // Handle null case
        totalRequests += parseInt(keyData.requests || 0);
        totalSuccessful += parseInt(keyData.successful_requests || 0);
        totalFailed += parseInt(keyData.failed_requests || 0);
        totalCost += parseInt(keyData.cost || 0);
      }
